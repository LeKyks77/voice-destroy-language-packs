[CmdletBinding()]
param(
    [string]$ReleaseTag = "packs-v1.0.0"
)
...
function Expand-VerifiedModel(
    ...
    try {
        foreach ($entry in $archive.Entries) {
            ...
            if ($segments.Count -eq 1 -or [string]::IsNullOrEmpty($entry.Name)) {
                continue
            }
            $relativePath = [string]::Join('/', $segments[1..($segments.Count - 1)])
            if (-not $seen.Add($relativePath)) {
                throw "Duplicate path in archive: $relativePath"
            }
...
function New-DeterministicZip([string]$SourceDirectory, [string]$DestinationZip) {
    ...
    try {
        $files = Get-ChildItem -LiteralPath $SourceDirectory -Recurse -File | Sort-Object { $_.FullName.Substring($sourceFull.Length).Replace('\', '/') }
...
function Test-PackArchive([string]$ArchivePath) {
    $required = @(
        ...
        "LICENSES/VOSK_MODEL_LICENSE.txt",
...
foreach ($languageId in @("fr_fr", "en_us")) {
    $source = $sourceIndex.$languageId
    $archivePath = Join-Path $downloadsRoot $source.archive
    if (-not (Test-Path -LiteralPath $archivePath -PathType Leaf)) {
        throw "Missing source model: $archivePath"
    }
    ...
    $sourceDirectory = Join-Path $repoRoot "sources\$languageId"
    ...
    $packRoot = Join-Path $workRoot $languageId
    ...
    Copy-Item -LiteralPath (Join-Path $sourceDirectory "pack.json") -Destination (Join-Path $packRoot "pack.json")
...
This language pack contains **$($pack.model.name)**, distributed by the Vosk project / Alpha Cephei.

- Upstream archive: $($pack.model.upstream_url)
- Source page: $($pack.model.source_page)
...
"@
    ...
    $outputName = "voice-destroy-language-$languageId-$($pack.version).zip"
    ...
    $manifestEntries.Add([ordered]@{
        ...
        sha256 = $outputHash
        license = [string]$pack.model.license
